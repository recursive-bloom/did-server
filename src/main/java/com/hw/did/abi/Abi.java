...
public class Abi extends Contract {
    ...
    public List<NodeInfoUpdatedEventResponse> getNodeInfoUpdatedEvents(TransactionReceipt transactionReceipt) {
        ...
        for (EventValuesWithLog eventValues : valueList) {
            ...
            typedResponse.info = (Node) eventValues.getNonIndexedValues().get(0);
            responses.add(typedResponse);
        }
        return responses;
    }
    ...
    public static class Node extends DynamicStruct {
        public byte[] parent;
        ...
        public Node(byte[] parent, String owner, BigInteger expire, BigInteger ttl, BigInteger transfer, String name) {
            super(new Bytes32(parent), new Address(owner), new Uint64(expire), new Uint64(ttl), new Uint64(transfer), new Utf8String(name));
            this.parent = parent;
            this.owner = owner;
            this.expire = expire;
            this.ttl = ttl;
        ...
        public Node(Bytes32 parent, Address owner, Uint64 expire, Uint64 ttl, Uint64 transfer, Utf8String name) {
            super(parent, owner, expire, ttl, transfer, name);
            this.parent = parent.getValue();
            this.owner = owner.getValue();
            this.expire = expire.getValue();
            this.ttl = ttl.getValue();
            this.transfer = transfer.getValue();
